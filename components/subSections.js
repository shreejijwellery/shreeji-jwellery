...
const SubSectionManager = (props) => {
    ...
    const fetchItems = async (isCallApi) => {
        setLoading(true); // Start loading
        try {
            const response = await fetchAllSubSections(isCallApi);
    ...
    const handleAddItem = async (e) => {
        ...
        setLoading(true); // Start loading
        try {
            const response = await HTTP('POST','/sub-section', payload);
            fetchItems(true);
    ...
    const handleEditItem = async (item) => {
        ...
        setLoading(true); // Start loading
        try {
            const response = await HTTP('PUT','/sub-section', item);
            // setItems(items.map((i) => (i._id === item?._id ? response.item : i)));
            fetchItems(true);
            toast.success('Sub Section updated successfully', { autoClose: 500 }); // Set autoClose to 1 second
        } catch (error) {
    ...
    const handleDeleteItem = async (itemId) => {
        ...
        setLoading(true); // Start loading
        try {
            await HTTP('DELETE','/sub-section', { id: itemId });
            setItems(items.filter((item) => item?._id !== itemId));
            fetchItems(true);
            toast.success('Sub Section deleted successfully', { autoClose: 500 }); // Set autoClose to 1 second
        } catch (error) {
