...
import { fetchAllParties, HTTP } from '../actions/actions_creators';
...
const PartyDetails = props => {
  ...
  const handleCreateWorker = async () => {
    setLoading(true);
    try {
      const response = await HTTP('POST', '/party', { ...newWorker });
  ...
  const handleDeleteWorker = async id => {
    setLoading(true);
    try {
      const response = await HTTP('DELETE',`/party/${id}`);
  ...
  const handleEditWorker = async () => {
    setLoading(true);
    try {
      const response = await HTTP('PUT',`/party`, editingWorker);
