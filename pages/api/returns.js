...
async function handler(req, res) {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      const data = await ReturnOrder.aggregate([
        { $match: query },
        {
          $group: {
            _id: {
    ...
    if (method === 'POST') {
      ...
      const result = await ReturnOrder.insertMany(ordersToInsert, { ordered: false });

      return res.status(201).json({
        success: true,
        message: `Successfully uploaded ${result.length} returns for ${startDate}`,
        count: result.length
      });
    }
    ...
    if (method === 'DELETE') {
      ...
      return res.status(200).json({
        success: true,
        message: `Deleted ${result.modifiedCount} returns for ${date}`,
